...
parser.add_argument('--n_epoch', type=int, default=1000, help='number of epochs to train for')
...
which contains expressions inside braces. The expressions are replaced
with their values.
'''
...
# The file where we save the output	
log_output = open(f"{logs_dir}/log.txt", 'w')
print(param)
print(param, file=log_output)

# For plotting the Loss of D and G using tensorboard
#TODO: from tensorboard_logger import configure, log_value
#TODO: configure(logs_dir, flush_secs=5)

# Improt CUDA if needed
if param.cuda:
...
class DCGAN_G(torch.nn.Module):
	def __init__(self):
		...
		''' A sequential container. Modules will be added to it in
...
class DCGAN_D(torch.nn.Module):
	def __init__(self):
		...
		# Size = 1 x 1 x 1 (Is it a real cat or a fakely genereated cat?)
		self.main = main
	...
	def forward(self, input):
		if isinstance(input.data, torch.cuda.FloatTensor) and param.n_gpu > 1:
			...
			output = self.main(input)
		# Convert from 1 x 1 x 1 to 1 so that we can compare to given label (Is it a real cat or a fakely genereated cat?)
		# It collapses all non-batch dimensions
...
'''normal_ fills self tensor with elements samples from the normal
...
# Now Variables
'''NOTE: These examples is not (!) updated for PyTorch 0.4,
which made several major changes to the core PyTorch API.
 Most notably, prior to 0.4 Tensors had to be wrapped in
 Variable objects to use autograd; this functionality has
 now been added directly to Tensors, and Variables are now deprecated.'''
x = Variable(x)
y = Variable(y)
z = Variable(z)
...
# betas[0] represents the weight given to the previous mean of the gradient
# betas[1] represents the weight given to the previous variance of the gradient
...
for epoch in range(param.n_epoch):
	...
	for i, data_batch in enumerate(dataset, 0):
		...
		# Mostly necessary for the last batch because N might not be a multiple of batch_size
		current_batch_size = images.size(0)
		if param.cuda:
			images = images.cuda()
		# Transfer batch of images to x
		x.data.resize_as_(images).copy_(images)
		# y is now a vector of size current_batch_size filled with real_label=1
		y.data.resize_(current_batch_size).fill_(real_label)
		y_pred = D(x)
		...
		x_fake = G(z)
		# y is now a vector of size current_batch_size filled with fake_label=0
		y.data.resize_(current_batch_size).fill_(fake_label)
		# Detach y_pred from the neural network G (stop backprop to the generator) and put it inside D
		# See https://github.com/pytorch/examples/issues/116 and https://github.com/yunjey/pytorch-tutorial/issues/56
		...
		# Log results so we can see them in TensorBoard afterwords
		#TODO:log_value('errD', errD.data[0], current_step)
		#TODO: log_value('errG', errG.data[0], current_step)
